from AlgorithmUtilities import canProceed, getAttr, getIdenName, retrieveInfos, removeAttr

def entityMatching(ideng, conn):
  #TODO: Riserva una parte al confronto degli attributi identificatori
  if canProceed(ideng, conn):
    attrG1, attrG2 = getAttr(ideng, "G1", conn), getAttr(ideng, "G2", conn) #gia' ordinati per chiave alfabeticamente
    toExaminG1, toExaminG2 = getIdenName(ideng, "G1", conn), getIdenName(ideng, "G2", conn) #gia' ordinati per chiave alfabeticamente
    toRemoveG1 = list()
      ...
      for keyG2 in attrG2:
        if keyG1 == keyG2:
          if attrG1.get(keyG1) == attrG2.get(keyG2):
            ...
            print("Attributo {} di G1 contraddittorio con attributo {} in G2, valore attributo in G1 --> {}, valore attributo in G2 -->  {}".format(keyG1, keyG2, attrG1.get(keyG1), attrG2.get(keyG2)))
    ...
    for key in attrG1:
      print("Attributo {} in G1 con valore {}, complementare".format(key, attrG1.get(key)))
    
    for key in attrG2:
      print("Attributo {} in G2 con valore {}, complementare".format(key, attrG2.get(key)))
