...
def sameSource(r1, r2, conn):
  q = "match (e) -[r]-> () where id(r) = " + str(r1) + " with e.id as id1 match (e) -[r]-> () where id(r) = " + str(r2) + " return e.id = id1"
  return conn.query(q).pop()[0] # type: ignore

#@title Funzione di creazione relazioni
def getLimit(tipo, conn):
  q = "MATCH (e) -[:HAS]-> (ee) WHERE e.label ='" + tipo + "' and labels(ee)[0] = 'Cardinalita' return properties(ee).label"
...
def isSemiFissa(tipo, conn):
  q = "match (e) -[:HAS]-> (something) where e.label = '" + tipo + "' and labels(something)[0] = 'Cardinalita' return something.label <> '1' and something.label <> 'n'"
  return conn.query(q).pop()[0] # type: ignore

#@param(tipo) il nome per trovarla nel metamodello
#return --> true se la relazione e' di tipo Fisso, false altrimenti
def isFissa(tipo, conn):
  q = "match (e) -[:HAS]-> (something) where e.label = '" + tipo + "' and labels(something)[0] = 'Cardinalita' return something.label = '1'"
  return conn.query(q).pop()[0] # type: ignore

def isMultipla(tipo, conn):
  q = "match (e) -[:HAS]-> (something) where e.label = '" + tipo + "' and labels(something)[0] = 'Cardinalita' return something.label = 'n'"
  return conn.query(q).pop()[0] # type: ignore

def canCreate(tipoT, attrT, grafoT, relName, conn):
  if not isMultipla(relName, conn):
    q = "MATCH () -[r:" + relName + "]-> (e:" + tipoT + ") WHERE e.graph = " + str(grafoT) + " and "
...
def create_relation_dir(typeES, ES_attr, gS, typeET, ET_attr, gT, relName, conn):
  with conn.driver.session(default_access_mode=neo4j.WRITE_ACCESS) as session: # type: ignore
    with session.begin_transaction() as tx:
      if checkInsertion(getId(typeES, ES_attr, gS, conn), getId(typeET, ET_attr, gT, conn), relName, conn):
...
def addRelAttribute(conn, type, attribute):
  with conn.driver.session(default_access_mode=neo4j.WRITE_ACCESS) as session:
    with session.begin_transaction() as tx:
      
      print("A new attribute named " + str(attribute) + " will be added to relation type " + str(type) + ". Continue?")
      print("y - yes, n - no")
      flag = input()

      if (flag == "y"):
        typePresence = tx.run("MATCH (instance:Relazione {type: 'relation', label:'" + str(type) + "'}) RETURN instance")
        typeValues = typePresence.values()

        if (len(typeValues) == 1):
          attrAlreadyPresent = tx.run("MATCH (instance:Relazione {type: 'relation', label:'" + str(type) + "'}) -[rel:HAS]-> (attr:" + str(attribute) + " {type: 'attr'}) RETURN instance, rel, attr")
          attrPresentValues = attrAlreadyPresent.values()

          if (len(attrPresentValues) == 0):
            attrCreate = tx.run("MATCH (type:Relazione{type: 'relation', label:'" + str(type) + "'}) CREATE (type) -[rel:HAS]-> (attr:" + str(attribute) + " {label: '" + str(attribute) + "', type: 'attr'}) RETURN type, rel, attr")
          ...
          print("Problem with relation type. Number of " + str(type) + " type discovered: " + str(len(typeValues)))
...
def alreadyExist(t1 : str, t2 : str, t : str, conn):
  q = "match (t1) match (t2) where t1.label = '" + t1 + "' and t2.label = '" + t2 + "' match (t1) -[r:" + t + "]- (t2) return count(r) = 1"
  return conn.query(q)[0][0] # type: ignore
...
def addConstraint(relType1 : str, relType2 : str, relType : str, conn):
  if not alreadyExist(relType1, relType2, relType, conn):
    q = "match (t1) match (t2) where t1.label = '" + relType1 + "' and t2.label = '" + relType2 + "' create (t1) -[r:" + relType + "]-> (t2) return count(r) = 1"
    return conn.query(q)[0][0] # type: ignore
  return False
...
def getContraddictory(par, t : int, conn):
  if t == 1: #tipo di relazione
    q = "match (e1) where e1.label = '" + par + "' match (e1) -[r:CONTRADDITTORI]- (e2) return e2.label as tipo"
  elif t == 0: #id di entita' di relazione
    q = "match (e) <-[r]- () where id(e) = " + str(par) + " with type(r) as t match (e) -[:CONTRADDITTORI]- (c) where e.label = t return c.label as tipo"
  else:
...
def srcCheck(entSrc: int, entDst: int, rel: str, conn):
  q = "match (e1) -[r:" + rel + "]-> (e2) where id(e1) = " + str(entSrc) + " and id(e2) = " + str(entDst) + " return count(r) > 0"
  return conn.query(q)[0][0] # type: ignore
...
def checkInsertion(entSrc: int, entDst : int, rel : str, conn):
  presentContr = getContraddictory(rel, 1, conn) #ottengo tutte le relazioni in contraddizioni con quella da inserire
