...
from Relation import getLimit, isSemiFissa
...
def getAttr(ideng, g, conn):  #ritorna una lista di chiavi valori
  d = {}
  q = "match (p) where " + whichGraph(g) + "p.id = " + str(ideng) + " return properties(p)"
...
def getIdenName(ideng, g, conn): 
  l = list()
  q = "match (p) where " + whichGraph(g) + "p.id = " + str(ideng) + " return labels(p)"
  result = conn.query(q)  # type: ignore
  if len(result) == 0:
    return l
  res = result.pop()[0][0] # type: ignore
  q = "match (:" + res + ") -[:IDENTIFIED]-> (:Identifier) -[:IDENTIFIED_BY]-> (p) return p.label as iden"
  result = conn.query(q)  # type: ignore
...
def getSemi(id, grafo, direzione, conn):
  ...
  if direzione == 0:
    ...
    q += "() -[r]-> (e) "
  q += "WHERE e.graph = " + str(grafo) + " AND e.id = " + str(id) + " RETURN id(r) as id, type(r) as tipo, id(startNode(r)) as da, id(endNode(r)) as a"
  result = conn.query(q)  # type: ignore
  
  while len(result) != 0: # type: ignore
    res = result.pop() # type: ignore
    l.append({'id':res.get('id'), 'tipo':res.get('tipo'), 'da':res.get('da'), 'a':res.get('a')})
...
def sameRel(r1, r2, id, direzione, conn):
  q = "MATCH " + getDir(direzione) + "where id(r) = " + str(r1) + " and e.id = " + str(id) + " with r as ee match " + getDir(direzione) + " where id(r) = " + str(r2) + " and e.id = " + str(id) + " return type(r) = type(ee)" 
...
def sameTarget(r1, r2, conn):
  q = "match () -[r]-> (e) where id(r) = " + str(r1) + " with e.id as id1 match () -[r]-> (e) where id(r) = " + str(r2) + " return e.id = id1"
  return conn.query(q).pop()[0] # type: ignore
...
def overLimit(tipo, val, l1, l2, conn):
  ...
  if  val > getLimit(tipo, conn):
    l.append("Relazione di tipo " + tipo + " contraddittoria, nei due grafi esplorati esistono ben " + str(val) + " di queste relazioni")
  else:
